...
for n in 1 2 3; do
    : > "$DIR/lane$n.log"
    tmux kill-session -t "e30$n" 2>/dev/null
    cat > "/tmp/e30lane$n.sh" <<EOF
#!/bin/bash
{
  echo "start=\$(date -u +%FT%TZ)"
  /root/bin/needle-board run $n -- bash -c "EXP30=1 LANE_OUT=$DIR/lane$n.out bash .auto/exp29/lane.sh $n"
  echo "LANE rc=\$?"
  sleep 60
} >> "$DIR/lane$n.log" 2>&1
EOF
    chmod +x "/tmp/e30lane$n.sh"
    tmux new-session -d -s "e30$n" -c "/root/board-pool/board$n" "/tmp/e30lane$n.sh"
done
sleep 12
echo "panes=$(tmux ls 2>/dev/null | grep -c e30)"
