...
python3 "$MAIN/.auto/exp44/apply_fwscale.py" "$S/nd_quant.c" || exit 1

python3 - "$S" <<'PY'
import pathlib, sys
s = pathlib.Path(sys.argv[1])
text = (s / "nd_quant.c").read_text()
i = text.index("static ND_HOT void nd_fwht2s")
j = text.index("\nstatic ND_HOT void fwht_rows")
body = text[i:j].replace("static ND_HOT void nd_fwht2s", "void nd_fwht2s", 1)
...
void nd_fwht2s(float *x0, float *x1, uint32_t n, float scale);
...
int main(void) {
    ...
    for (int gi = 0; gi < 6; gi++) {
        ...
        for (int trial = 0; trial < 400; trial++) {
            float a[256], b[256], ra[256], rb[256];
            ...
            nd_fwht2s(a, b, g, scale);
            for (uint32_t i = 0; i < g; i++) {
                if (memcmp_(&a[i], &ra[i]) || memcmp_(&b[i], &rb[i])) bad++;
                cases++;
            }
        }
    }
