...
void app_main(void)
{
    ...
    /* Diagnostic (run #447, corrected): the REAL cost of one nd_parallel_rows
     * split, measured where nd_parallel_rows is genuinely rows_dual_core. #344's
     * 23-cycle figure is void: a kbench image never reaches worker_start(), so it
     * timed a serial indirect call. Three corrections from review: no shared
     * accumulator (each core writes ONLY its own slot, which also proves both
     * halves ran), a parked variant (the worker is blocked on s_go when the
     * timed call starts, i.e. the tap96 case), and an empty body alongside a
     * busy one so the per-unit slope is separable from the handshake. */
    {
        static volatile unsigned long s_w2[4];   /* [busy?2:0][core] */
        static volatile unsigned long s_sink1;
        void sp_empty(void *vc, uint32_t a, uint32_t b) { (void)vc; (void)a; (void)b; }
        void sp_busy(void *vc, uint32_t a, uint32_t b) {
            (void)vc; unsigned long k = 0;
            for (uint32_t i = a; i < b; i++) k += (unsigned long)i * 3u + 1u;
            s_w2[2 + (unsigned)xPortGetCoreID()] += k;   /* per-core slot, no race */
        }
        const uint32_t units[] = { 4u, 8u, 32u, 128u };
        for (unsigned v = 0; v < 2; v++) {
            nd_row_fn fn = v ? sp_busy : sp_empty;
            for (unsigned p = 0; p < 2; p++) {           /* 0 = hot peer, 1 = parked */
                for (unsigned ui = 0; ui < 4; ui++) {
                    uint32_t u = units[ui];
                    unsigned long bsp = ~0ul, bser = ~0ul;
                    for (int r = 0; r < 33; r++) {
                        if (p) vTaskDelay(pdMS_TO_TICKS(20));   /* worker blocks again */
                        uint32_t t0 = esp_cpu_get_cycle_count();
                        nd_parallel_rows(fn, 0, u);
                        unsigned long d = (uint32_t)esp_cpu_get_cycle_count() - t0;
                        if (d && d < bsp) bsp = d;
