...
extern void nd_fwht4r(float *x0, float *x1, float *x2, uint32_t n);
/* Set once by nd_model_open's acceptance test (nd_model.c); 0 keeps the C walk. */
extern int nd_fwht4r_ok;
#endif
...
#if defined(ND_GEMV4_ASM)
    /* E59: the first two stages have contiguous cells, so one 128-bit load and
     * one store per group cover a 4-cell block per pass. Every output is the same
     * t1+t3 / t2+t4 expression from the same adds and subs, so the values are
     * identical to running len=1 then len=2; the C loop resumes at len=4. The
     * verdict is printed at boot - a guard that fails silently measures as a null. */
    if (nd_fwht4r_ok && n >= 8u &&
        (((uintptr_t)x0 | (uintptr_t)x1 | (uintptr_t)x2) & 15u) == 0u) {
        /* One line, once: a guard that falls back silently measures as a null, so
         * the field must say which arm ran and with what alignment. */
        static int seen;
        if (!seen) {
            seen = 1;
            printf("FWHT4R live=1 n=%u align=%u\n", (unsigned)n,
                   (unsigned)(((uintptr_t)x0 | (uintptr_t)x1 | (uintptr_t)x2) & 15u));
        }
        nd_fwht4r(x0, x1, x2, n);
        len = 4u;
    } else
#endif
    len = 1u;
...
static nd_row_fn gemv4_pick(const gemv_ctx *c, const void *blob, uint32_t rows)
{
    ...
    /* the wide twin needs 16-byte-aligned xh (ND_ALLOC_FAST16); the check keeps
     * a misaligned operand on the proven plain kernel instead of falling back
