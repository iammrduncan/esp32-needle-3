...
## CURRENT STATE AND QUEUE -- 2026-09-26T02:45Z -- AUTHORITATIVE

**Accepted runtime (owner's pin): 5.3033 decode tok/s** (bundle5, 20/20 device goldens, 19/19
host, fidelity 5.341e-05). **Two fully-evidenced proposals await owner admission:**

1. **SHIPPABLE LINE (recommended: zero unaccepted engine code + main.c transport/scheduler):**
   bundle5 + composed attention family (B4W paired-head shared-V P.V + DOT8W 8-column QK body
   + SELRES selective rescale sweeps) + noinline KV-staging helper + late lossless RX ring
   (fixes the 17-request console wedge: case 17 is the first >128-byte request and the polled
   VFS FIFO drops bytes) + ported spin handshake = **5.5450 screen (+4.56 % over the pin)**,
   full gate of the pre-spin form 5.4650 (+3.048 %), ext 5.3992 (+3.29 %), think 4.30, min_case
   5.26-5.34, boot bench 180 ms, capture GREEN on the tree (CAP-shp-b3 rc=0), host 19/19
   byte-exact, all 20 cases complete. Worker/b2 spin knobs bracketed null at the seed pins
   (#657). The b1 full gate + b3 screen of the +spin form are running now.
2. **SEED-ERA LINE (max speed, includes the unaccepted quant/asm stack):** composition +
   staging helper = **5.8083 / 5.8050** on two boards' FULL gates, ext 5.7331, think 4.51,
   boot 170 ms/token, heap 5087, capture GREEN on the tree, host 19/19.

**THE only outstanding gate on both lines: two demo-timer goldens** (heldout_interval_one,
heldout_long_tools_note_only) show 18/20 with token_delta 52 on every image that carries the
RX ring - and run #647 PROVED a pure transport cause: bundle5 + ring-only, engine byte-pure
accepted, flips exactly that pair while the same engine without the ring is 20/20. Owner
options: (a) accept the pair as timing-state-sensitive and re-baseline, (b) replace the two
cases with state-independent ones, (c) keep them as blockers and ship nothing with the ring
(which also means no 20-case suite and no >128 B requests).

**Harness state:** pins now 5.8083 seed / 5.4650 shippable (5.5450 pending gates); worker
b2 = seed pin 91e79eeeb304, b1+b3 = shippable+spin e6d55f29e286. measure.sh prints
PROVENANCE and honours EXPECT_ENGINE_MD5; restricted (primary-only) runs are device-gated
(#595/#598 proved it red, #604 green). Coverage additions are blocked while the device
fixtures differ from main's - see #467's atomic recipe before touching prompts.json.

**Live queue after the running gates:** (1) if the +spin gates reproduce, re-issue the
shippable packet at +4.5 % and hand the two-golden decision to the owner - DO NOT AUTO_SAVE
the goldens (the recipe forbids it: the two demo-timer entries must never be silently
re-captured). (2) spin idle-power cost is a documented open question for the owner
(worker spins ~200 us before blocking; correctness independent of the spin). (3) Candidate
pool above the sub-quantum is EMPTY: width axis closed on both sides of both peaks (P.V 4,
QK 8, staging/sweeps null), spill class closed (whole-ELF scan clean), cold/code-size class
closed three times, wait-knobs bracketed, delivery/instruction floors unchanged. New
hypotheses need a phase with a FRESH measurement, not a re-screen.
