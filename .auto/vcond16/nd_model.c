            ...
            return -11;
        {
            ...
            for (f = 0; f < (int)(sizeof(SLOT) / sizeof(SLOT[0])); f++)
                for (li = 0; li < m->n_layers; li++) {
                    ...
                    /* Slot 25 is cond_v. Keep it as the halfwords the archive already holds: the
                     * only reader is cond_rows (verified single consumer), the row length is 8 so
                     * n/2 is integral, and nd_f16(h[k]) is the same float this loop wrote. The
                     * pool's size computation is deliberately untouched, so the pool just ends up
                     * with ~98 KB reserved and unused. */
                    if (SLOT[f] == 25) {
                        uint16_t *q16 = (uint16_t *)p;
                        for (k = 0; k < n; k++)
                            q16[k] = h[k];
                        p += n / 2;
                    } else {
...
/* cv is the staged cond_v table kept as fp16 rather than float; see the pool fill in
 * nd_model_open. nd_f16 of a stored halfword returns exactly the float staging wrote, so
 * every product below is bit-identical. */
typedef struct { float *dst; const float *x; const uint16_t *cv; uint32_t dm; } cond_ctx;
...
static ND_HOT void cond_rows(void *vc, uint32_t c0, uint32_t c1)
{
    ...
    for (ch = c0; ch < c1; ch++) {
        float acc = 0.0f;
        for (i = 0; i < c->dm; i++)
            /* Runs #380/#381 measured this loop as bound by these strided loads, not by the
             * serial sum, so shrinking the operand is the lever and interleaving channels
             * was not. Stride 8 floats means 2 useful floats per 64 B line; as fp16 it is 4,
             * and the table the loop re-sweeps per channel halves from 24 KB to 12 KB. */
            acc += c->x[i] * nd_f16(c->cv[(size_t)i * 8 + ch]);
        c->dst[ch] = acc;
    }
}
...
static void hadamard_mlp_unscaled(nd_model *m, uint32_t li, const float *x)
{
    ...
    const float *d1 = fp[14];
        const float *d2 = fp[15], *b2 = fp[16], *d3 = fp[17];
    /* cond_v's slot holds halfwords now (see the pool fill), so the slot must be read as
     * uint16_t here too. The host build accepted the float* form as a warning - same pointer
     * value, so it still measured the right bytes - and the xtensa build refuses it. */
    const uint16_t *cv = (const uint16_t *)fp[25];
