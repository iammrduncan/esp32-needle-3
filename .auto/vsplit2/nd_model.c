...
static void attention(nd_model *m, uint32_t li, const float *xin, float *out)
{
    ...
    { ND_T0(tt);
    ND_T0(tq); tap_projection(m, 4, m->q, m->q_hist, li, nh * qk_hd); ND_T1(tq, ND_P_TAPS_Q);
    ND_T0(tk); tap_projection(m, 5, m->kbuf, m->k_hist, li, m->k_dim); ND_T1(tk, ND_P_TAPS_K);
    ND_T0(tv); tap_projection(m, 6, m->vbuf, m->v_hist, li, m->v_dim); ND_T1(tv, ND_P_TAPS_V);
    ND_T1(tt, ND_P_TAPS); }
