...
typedef struct {
    ...
    nd_tensor q_norm, k_norm;
    nd_tensor gate_proj, out_proj, post_norm, attn_gate, pre_hada;
    nd_tensor d1, d2, b2, d3, d4;
    nd_tensor w1a, w1b, w2a, w2b, w3a, w3b, cond_v, cond_u;
} nd_layer;
...
typedef struct {
    ...
    uint32_t      d_model, n_layers, n_heads, n_kv_heads, qk_head_dim, v_head_dim;
    uint32_t      attn_dim, k_dim, v_dim, lanes, vocab, window;
    ...
    /* Legacy confidence-head representation. Needle 3's six-tensor head is
     * not yet wired into this port. Pooling is a
    ...
    int8_t       *k_cache;      /* [layer][window][k_dim] */
    int8_t       *v_cache;
    ...
    float        *snap_eg_hist;
    float        *snap_q_hist, *snap_k_hist, *snap_v_hist;
...
enum { ND_P_PROJ, ND_P_ATTN, ND_P_MLP, ND_P_PHI, ND_P_ENGRAM,
       ...
       /* ND_P_TAPS is 3.3 ms/token (1.7 %) and is the largest phase with no internal
        * split left, now that mhc-mix (2.5 ms) and sinkhorn (3.8 ms, priced to the
        * transcendental) are both attributed. The bracket covers three tap_projection
        * calls (q, k, v) over 9 taps x their own width; only the q table is wide, so if
        * the phase is not q, the lever is elsewhere. Sub-phases, additive. */
       ND_P_TAPS_Q, ND_P_TAPS_K, ND_P_TAPS_V,
       ND_P_COUNT };
