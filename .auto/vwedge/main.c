...
void app_main(void)
{
    ...
    for (;;) {
        ...
        /* Diagnostic build, NOT for shipping: the campaign has eight attempts where the board
         * answers 16-17 requests from one boot and then stops answering, and the accepted image
         * reproduces it, so it is a firmware/console defect rather than a candidate. What is not
         * known is WHICH half stops: this heartbeat distinguishes "no bytes ever reach the app"
         * (a console-leg/USB problem, or a host-side framing problem in my own reader) from
         * "bytes arrive and a line completes but nothing is answered" (an app-side block after
         * the read). eof/chars/lines plus free heap in one line, so a wedged session can be read
         * from the log alone. */
        static unsigned long s_rx_chars = 0, s_rx_lines = 0, s_rx_eof = 0;
        /* Read one request line from the console. */
        for (;;) {
            int c = getchar();
            if (c == EOF) {
                /* Candidate fix, and the diagnostic says whether it worked: newlib latches the
                 * EOF indicator on a FILE once a read returns no data, and every later getchar()
                 * then returns EOF WITHOUT consulting the driver again. That matches every
                 * observation this campaign has of the mid-suite wedge: the reader task is alive
                 * and spinning (eof climbs), chars/lines freeze at their last values, bytes the
                 * host writes never reach the app, reconnects change nothing, and only a chip
                 * reset restores service (#391). clearerr() is the one-line un-latch, so a wedge
                 * that clears is that mechanism and a wedge that persists is not. */
                if ((s_rx_eof % 25u) == 0u)
                    clearerr(stdin);
                if ((++s_rx_eof % 250u) == 0u) {
                    printf("EVT RX_IDLE eof=%lu chars=%lu lines=%lu free_int=%u free_ps=%u\n",
                           s_rx_eof, s_rx_chars, s_rx_lines,
                           (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
                           (unsigned)heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
                    fflush(stdout);
                }
        ...
        if (len == 0)
            continue;
        s_rx_lines++;
        printf("EVT RX_LEN n=%d lines=%lu\n", len, s_rx_lines);
        fflush(stdout);
