/* nd_cact.h - reader for the Needle 3 `.cact` deployment blob.
 *
 * Layout (little-endian):
 *   0x00  196-byte header (see nd_cact_header)
 *   0xC4  codebook    codebook_len * f32 = cb2[4] | cb3[8] | cb4[16]
 *          directory   num_tensors * 44-byte records
...
#define ND_CACT_TAG      0x05E12A84u
...
/* Geometry as stored in the Needle 3 header. Field order is on-disk order. */
typedef struct {
    uint32_t tag;
    ...
    uint32_t global_mask_lo;
    uint32_t global_mask_hi;
    uint32_t qkv_conv_taps;
    uint32_t engram_slots;
    uint32_t engram_sub_dim;
    uint32_t engram_tables;
    uint32_t engram_conv_taps;
    uint32_t engram_dilation;
    uint32_t engram_seed_heads;
    uint32_t num_orders;
    uint32_t orders[4];        /* n-gram orders, zero-padded */
    uint32_t num_sites;
    uint32_t sites[16];        /* layer indices the engram fires at */
    float    rope_theta;
} nd_cact_header;
