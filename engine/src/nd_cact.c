...
/* The header is a flat run of 49 little-endian u32-sized fields and
...
int nd_cact_open(nd_cact *c, const void *blob, size_t size)
{
    ...
    if (c->h.d_model == 0 || c->h.num_layers == 0 || c->h.num_heads == 0 ||
        c->h.num_kv_heads == 0 || c->h.qk_head_dim == 0 || c->h.v_head_dim == 0 ||
        c->h.num_heads % c->h.num_kv_heads != 0 ||
        c->h.num_heads * c->h.v_head_dim != c->h.d_model ||
        c->h.num_orders > 4 || c->h.num_sites > 16 ||
        c->h.hada_n < c->h.d_model)
        return -3;
